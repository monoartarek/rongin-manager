...
<EditSection title="Reseller Settings">
  <div className="flex items-center gap-3 mb-4">
    ...
    <div>
      ...
      <p className="text-xs text-slate-500">
        Toggle the checkbox in Roles section above to change reseller status
      </p>
    </div>
  </div>
  ...
  <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4">
    <EditField label="Reseller Coins">
      <input
        ...
        type="number"
        min="0"
        value={user.reseller_coins ?? 0}
        onChange={e => set("reseller_coins", Number(e.target.value))}
        placeholder="0"
      />
    </EditField>
    <EditField label="WhatsApp Number">
      <input
        className={iCls}
        type="text"
        value={user.reseller_whatsAppnumber || ""}
        onChange={e => set("reseller_whatsAppnumber", e.target.value)}
  ...
  {user.isreseller && (
    <div className="mt-2 flex items-center gap-2 text-xs text-amber-400 bg-amber-500/10 border border-amber-500/25 rounded-lg px-3 py-2">
      <span>◈</span>
      <span>Reseller is active — coins and WhatsApp number will be saved.</span>
    </div>
  )}
</EditSection>
...
q.select("uid","name","username","gender","status","email","birthday","avatar","createdAt","updatedAt",
  ...
  "list_of_images","isreseller","reseller_coins","reseller_whatsAppnumber","isViewer","Room_priority"
);
 ...
 const saveEdit = useCallback(async (editedUser) => {
  ...
  try {
    ...
    const direct = [
      ...
      "country", "bio", "gender", "bkash_number", "tag",
      "user_designation", "admin_role", "agency_name", "agency_role",
      ...
      "isreseller", "reseller_whatsAppnumber",
      "isViewer", "Room_priority",
      "diamondsTotal", "total_recharged_credits",
    ];
    direct.forEach(k => { if (editedUser[k] !== undefined) obj.set(k, editedUser[k]); });

    /* ── numbers that need conversion ── */
    if (editedUser.uid !== undefined)
      obj.set("uid",            Number(editedUser.uid)            || 0);

    if (editedUser.credit !== undefined)
      obj.set("credit",         Number(editedUser.credit)         || 0);

    if (editedUser.diamonds !== undefined)
      obj.set("diamonds",       Number(editedUser.diamonds)       || 0);

    if (editedUser.creditSent !== undefined)
      obj.set("creditSent",     Number(editedUser.creditSent)     || 0);

    /* ── reseller_coins saved here ── */
    if (editedUser.reseller_coins !== undefined)
      obj.set("reseller_coins", Number(editedUser.reseller_coins) || 0);

    /* ── date ── */
    if (editedUser.birthday)
      obj.set("birthday", new Date(editedUser.birthday));

    /* ── avatar ── */
    if (editedUser.avatar === null)
      obj.unset("avatar");
    else if (editedUser.avatar instanceof Parse.File)
      obj.set("avatar", editedUser.avatar);

    /* ── party theme ── */
    if (editedUser.using_party_theme === null)
      obj.unset("using_party_theme");
